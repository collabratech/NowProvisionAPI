...
{
    using MassTransit;
    using NowProvisionAPI.WebApi.Extensions.Services.ProducerRegistrations;
    using NowProvisionAPI.WebApi.Extensions.Services.ConsumerRegistrations;
    using Messages;
            ...
            if (!configuration.GetValue<bool>("UseInMemoryBus"))
            {
                services.AddMassTransit(mt =>
                {
                    ...
                    mt.UsingRabbitMq((context, cfg) =>
                    {
                        ...
                        cfg.SubmitReportRequest();

                        // Consumers -- Do Not Delete This Comment
                        cfg.AllReportsGetSentFromHereEndpoint(context);
                    });
                });
